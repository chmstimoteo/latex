...
\subsection{Análise Quantitativa de Riscos}
...
\section{Técnicas Convencionais para Análise de Risco}
...
%\subsection{Fault Tree Analysis - Fault Mode and Effects Analysis}
%Fault Tree Analysis (FTA) é um método importante para analisar e estimar a confiabilidade e disponibilidade de um sistema. No processo de concepção do sistema, depois de analisar vários fatores de fracasso (como \textit{hardware}, \textit{software}, ambiente, fatores individuais), podemos construir o diagrama lógico (isto é, a árvore de falha), sequencialmente, identificando todas as causas para prever a probabilidade de falha do sistema e tomar medidas corretivas para melhorar a confiabilidade e segurança \cite{yu2009application}.
%Fault Tree Analysis (FTA) is an important tool to analyze and estimate system reliability and availability. In system design process, after analyzing several failure factors (such as hardware, software, environment, individual factors) we can build the logic diagram (that is to say, the fault tree), sequentially, identifying all causes to predict probability of system failure and taking corrective actions to improve reliability and security \cite{yu2009application}.
...
%In the process of fault tree construction, an undesired effect is taken as a target in logical analysis events, referred to as "top event"; and then every possible direct causes of such undesired effect are discovered, labeled as "middle events"; sequentially, every possible direct causes of those "middle events", are named "bottom events". The logical tree diagram, which is normally written using symbols of logic gates and top/middle/bottom events, is called Fault Tree \cite{yu2009application}.
%De modo geral, a construção da Árvore de Falha é dividida em quatro passos: 
%Typically, Fault Tree construction is divided into four steps:
%\begin{enumerate}
...
%\end{enumerate}
%\begin{enumerate}
%  \item Select and obtain the top event. The top event is the undesired event, or the failure event of the logical analysis in highlighted line.
%  \item Analyze the top event. Discover all direct, necessary and sufficient causes of the top event. Take the top event as the output event and all its direct reasons as input events, and interconnect those events through appropriate logic gates according to their current logical relationships.
%  \item Analyze all inbound events directly connected to the top event. Take them like output events in the next level if the event can be broken down further, as conducted in step 2.
%  \item Repeat the steps above, break them down step by step until every inbound event must not be broken. 
%\end{enumerate}
%In Figure \ref{fig:faulttree}, observa-se uma árvore de falha com um "evento de topo", três "eventos do meio" (A1,A2 e A3), "seis eventos" de fundo (B1 a B6), uma porta lógica "E" (T1) e três portas lógicas "OU" (T2, T3 e T4). Essa árvore de falha representa o seguinte conhecimento: O evento de topo ocorre, se os eventos A1, A2 e A3 ocorrerem. O evento A1 ocorre, se os eventos B1 ou B2 ocorrerem. O evento A2 ocorre, se os eventos B3 ou B4 ocorrerem. O evento A3 ocorre, se os eventos B5 ou B6 ocorrerem.
%In Figure \ref{fig:faulttree}, there is a completely reversed fault tree \cite{yu2009application}. The fault tree contains one "top event", three "middle events" (A1, A2 and A3), six "bottom events" (B1 to B6), a logic gate "AND" (T1) and three logic gates "OR" (T2, T3 and T4). Such fault tree represents the following concern: The top event occurs if the events A1, A2 and A3 arise. The event A1 occurs if the events B1 or B2 arise. The event A2 occurs if the events B3 or B4 arise. The event A3 occurs if the B5 or B6 events arise.
%\begin{figure}[H]
%	\centering
...
%	\label{fig:faulttree}
%\end{figure}
\section{Técnicas Estatísticas e de Computação Inteligente para Análise de Risco}
...
%Bayesian network belongs to the family of probabilistic graphical models. A Bayesian network $B$ is a direct acyclic graph (DAG) representing a combined probability distribution over a set of random variables $V$, in which the nodes represent variables and edges express the dependencies between variables \cite{jensen1996introduction}. Therefore, the network can be defined by a pair $B = <G,\Theta>$, where $G$ is the DAG whose nodes $X_1, X_2, ..., X_n$ represent random variables and whose edges represent direct dependencies between variables. Based on independent premises, each node $X_i$ in the graph $G$ is dependent on their parents in $G$. Therefore, the graph $G$ takes into account the Markov Assumption \cite{li2012integrated}. The Markovian Assumption for a Bayesian network states that any node in this network is conditionally independent of its non-descendants, given that there is a relationship. $\Theta$ represents the set of parameters of the Bayesian network, which contains parameters $\theta_{X_i|\pi} = P_B(X_i|\pi_i)$ for each value $x_i$ of the node $X_i$ conditioned on $\pi_i$, where $\pi_i = parent(X_i)$ \cite{neal1993probabilistic}.
...
%This technique has been used as an important tool for exploring various informations, including deterministic or probabilistic information from complex relationships between variables. They constitute a convenient research framework that is easy to use by specialists, mainly in reliability area . In particular, significant research interests exist in the use of Bayesian network models for reliability analysis of systems and risk in software engineering, but also quality forecast \cite{li2012integrated} \cite{fenton2007predicting}.
...
%Through the Bayesian network learning, all conditional probabilities between each two adjacent risks can be calculated. A Bayesian network may not only capture the relationship between unclear beliefs in propositions, but may also explain the truth value of one or more propositions. Algorithms of Bayesian network inference can be used to find causalities or current beliefs for each variable, and current relationships between variables \cite{jensen2007bayesian}. A Bayesian network encodes the probability distribution of a set of random variables by specifying a set of assumptions of conditional independence or dependence together with a set of relationships among those variables and their related joint probabilities. Instead of deterministic results typically generated by learning, Bayesian networks may produce a set of probabilistic results in each process. They are better suited for risk identification in several stages of the software process in real situations \cite{li2012integrated}.
...
%\subsubsection{Fuzzy Weight Average}
...
Em \cite{HUHUANG2007}, algumas técnicas inteligentes são utilizadas para a análise do risco de projetos de \textit{software}. O artigo conclui que uma técnica híbrida de redes neurais e algoritmos genéticos obtém uma precisão de 85\% na previsão do projeto obter sucesso, ser desafiado ou falhar totalmente.
...
\subsubsection{Aprendizado por Correção de Erro}
...
\subsubsection{Aprendizado por Broyden-Fletcher-Goldfarb-Shanno \textit{Backpropagation}}
...
Por simplicidade, nós assumimos que o sistema de inferência \textit{fuzzy} em consideração tem duas entradas $x$ e $y$ e uma saída $z$. Para um modelo \textit{fuzzy} de Sugeno de primeira ordem, um conjunto de regras comum com duas regras \textit{fuzzy} if-then são as sequintes:
...
A extensão do ANFIS Sugeno para Tsukamoto é direta, em que a saída de cada regra $(f_i, i = 1,2)$ é induzida conjuntamente por um conseqüente função de pertinência e uma força de disparo. Para o sistema de inferência \textit{fuzzy} Mamdani com a composição max-min, um ANFIS correspondente pode ser construído se aproximações discretas são utilizadas para substituir as integrais no esquema de defuzificação centróide. No entanto, o ANFIS resultante é muito mais complicado do que qualquer ANFIS Sugeno ou Tsukamoto. A complexidade extra na estrutura e cálculo de Mamdani ANFIS com composição max-min não implica, necessariamente, uma melhor capacidade de aprendizagem ou a aproximação de energia \cite{jang1997neuro}.
